	...
	if(JVERSION>=1.6)
	{
			class JFormFieldAupfilesocialads extends JFormField
			{
				...
				if($this->id=='jform_params_aupfilesocialads')
					{
					echo '<div style="float:left"><a href="'.JURI::root().'plugins/payment/altauserpoints/altauserpoints/socialads_aup.zip"> '.JText::_('AUP_CLK').'</a><span> '.JText::_('AUP_INST').' </span><a href="'.JURI::base().'index.php?option=com_altauserpoints&task=plugins" target="_blank">'.JText::_('HERE').'</a>
	...
	else
	{
		class JElementAupfilesocialads extends JElement
		{
			...
			function fetchElement($name, $value, &$node, $control_name)
			{
				...
				{
				return '<div style="float:left"><a href="'.JURI::root().'plugins/payment/altauserpoints/socialads_aup.zip"> '.JText::_('AUP_CLK').'</a><span> '.JText::_('AUP_INST').' </span><a href="'.JURI::base().'index.php?option=com_altauserpoints&task=plugins" target="_blank">'.JText::_('HERE').'</a>
