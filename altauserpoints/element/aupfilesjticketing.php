	...
	if(JVERSION>=1.6)
	{
			class JFormFieldAupfilesjticketing extends JFormField
			{
				...
				if($this->id=='jform_params_aupfilesjticketing')
					{
					echo '<div style="float:left"><a href="'.JURI::root().'plugins/payment/altauserpoints/altauserpoints/jticketing_aup.zip"> '.JText::_('AUP_CLK').'</a><span> '.JText::_('AUP_INST').' </span><a href="'.JURI::base().'index.php?option=com_altauserpoints&task=plugins" target="_blank">'.JText::_('HERE').'</a>
	...
	else
	{
		class JElementAupfilesjticketing extends JElement
		{
			...
			function fetchElement($name, $value, &$node, $control_name)
			{
				...
				{
				return '<br><div style="float:left"><br><a href="'.JURI::root().'plugins/payment/altauserpoints/jticketing_aup.zip"> '.JText::_('AUP_CLK').'</a><span> '.JText::_('AUP_INST').' </span><a href="'.JURI::base().'index.php?option=com_altauserpoints&task=plugins" target="_blank">'.JText::_('HERE').'</a>
